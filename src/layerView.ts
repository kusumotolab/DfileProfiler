...
export class LayerView {
    ...
    resizeScript = ''; // 画面の拡大縮小に対応するスクリプト
    canvasScript = ''; // レイヤビューのキャンバスを処理するスクリプト
    componentsScript = ''; // レイヤビューの構成要素を管理するスクリプト
    componentArray: LayerViewComponent[];
    totalComponent: LayerViewComponent | undefined;
    stateArray: State[]; // ビルド結果の履歴
    radioIndex = 0; // 選択中のラジオボタン
    radioCnt = 0; // 描画しているラジオボタンの数
    comment: string; // ビルド時に入力したコメント
    commentArray: string[]; // 過去のビルド時に入力したコメント履歴
    ...
    setComponentsScript() {
        ...
        this.componentArray.forEach(component => {
            tmpComponentsScript += ('{ x: ' + 0
                + ', y: ' + (component.sizeRectangle.y - 7 / 2) // ※7 => drawerのlineSpaceと一致させる
                + ', width: canvasCommon.width'
                + ', height: ' + (component.sizeRectangle.height + 7) // ※7 => drawerのlineSpaceと一致させる
                + ', color: ' + (component.index % 2 === 0 ? '\'whitesmoke\'' : '\'white\'') // 背景色を白とグレーの交互に設定
                + ' },\n');
        });
